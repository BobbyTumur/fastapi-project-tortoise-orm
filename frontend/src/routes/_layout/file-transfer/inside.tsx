...
import FileUpload from "../../../components/FileTransfer/FileUpload";

export const Route = createFileRoute("/_layout/file-transfer/inside")({
...
function RouteComponent() {
  ...
  const { data: objects, isLoading } = useQuery<Array<S3Object>>({
    ...
    queryFn: async () =>
      FileTransferService.listFiles({ folder: "to_customer" }),
  });

  return (
    <Container maxW="full">
      <Heading size="lg" textAlign={{ base: "center", md: "left" }} pt={12}>
        {t("titles.fileToBeTransferred")}
      </Heading>
      <Navbar text={t("titles.uploadFile")} addModalAs={FileUpload} />
      {isLoading ? (
            ...
            return (
              <Card size="md" variant="outline" key={obj.Key}>
                <CardHeader as={Flex} flexDirection="row">
                  ...
                  <Spacer />
                  <MenuFile fileKey={obj.Key} urlGenerationShown={true} />
                </CardHeader>
