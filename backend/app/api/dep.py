...
def get_current_permitted_user(current_user: CurrentUser) -> User:
    if not current_user.can_edit and not current_user.is_superuser:
        raise HTTPException(
            status_code=403, detail="The user doesn't have enough privileges"
        )
    return current_user

PermittedUser = Annotated[User, Depends(get_current_permitted_user)]